...
src/
  ...
  tts.py              # PiperTTS
  mpd_client.py       # MPDClient (implements MediaClientInterface; also MockMediaClient)
...
tests/
  ...
  test_menu.py
  test_session.py
  test_web.py         # Flask REST API tests; no systemd/sudo required
...
main.py
  └── session         (lifecycle: GPIO events → menu state machine)
        ...
        └── menu          (state machine; uses only interfaces, never concrete impls)
              ...
              │     └── media_client  (MediaClientInterface — MPDClient)
...
- **Genre media_key encoding** — genre `MediaItem.media_key` is stored as `"genre:{name}"` (e.g., `"genre:Jazz"`); `MPDClient.get_tracks_for_genre` parses the name from the key
...
- **`media_store`** (SQLite, `src/media_store.py`): persistent cache of playlists/artists/genres/albums; survives restarts; lazy-loaded, updated on successful media client responses, unchanged on errors
...
See `IMPL.md` § *Development Order* — interfaces first, then `error_queue`, `phone_book`, `gpio_handler`, `audio`, `tts`, `mpd_client`, `media_store`, `menu`, `session`, `main`.

## Test Strategy

- Unit tests inject mocks directly (no patching needed — ABCs are the seam)
- Integration tests are tagged and skipped by default; they hit a real MPD/Mopidy server
- GPIO is abstracted so `gpio_handler` can be driven by a mock pin reader
- Shared fixtures: `mock_gpio`, `mock_audio`, `mock_tts`, `mock_media_client` (also `MockMediaClient`), `mock_media_store` (also `MockMediaStore`), `tmp_phone_book`, `tmp_media_store`
...
- Actual MPD/Mopidy server responses (covered by integration tests, skipped by default)
- TTS audio quality
...
- **Use the `tmp_*` fixtures** — always use `tmp_phone_book`, `tmp_media_store` (for `MediaStore`), and
