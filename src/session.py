...
closed when it is replaced. Starts the GPIO digit-polling loop.
...
from src.dialer import Dialer, DialEvent
...
class Session:
    ...
    def __init__(
        ...
        if now is None:
            now = time.monotonic()
        self._menu = menu
        self._dialer = Dialer(pulse=pulse)
    ...
    def handle_event(
        self,
        event: Union[DialEvent, Tuple[DialEvent, int]],
        now: Optional[float] = None,
    ) -> None:
    ...
    def _tick_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                now = time.monotonic()
                if self._dialer is not None:
                    for digit, digit_now in self._dialer.drain_digits():
