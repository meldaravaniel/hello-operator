...
# ---------------------------------------------------------------------------
# 2.8 SD pin GPIO control
# ---------------------------------------------------------------------------

class TestSdPinControl:
    """SD pin GPIO control eliminates the I2S startup transient.

    SounddeviceAudio accepts sd_pin=<bcm> and _gpio_output=<callable(pin,val)>.
    When sd_pin is set the sequence in __init__ is:
      1. _gpio_output(sd_pin, 0)  — amp shutdown before aplay starts
      2. aplay process started    — I2S clock begins
      3. warmup silence written   — silence flows through the pipe
      4. sleep _SD_SETTLE_MS ms   — wait for silence to reach hardware
      5. _gpio_output(sd_pin, 1)  — amp powers up into stable, silent I2S
    """

    def test_sd_pin_driven_low_before_aplay(self):
        """SD pin driven LOW before aplay process is started."""
        call_log = []
        local_proc = FakeProcess()
        ...
        def fake_gpio(pin, value):
            call_log.append(('gpio', pin, value))

        a = SounddeviceAudio(sd_pin=22, _popen=tracking_popen, _gpio_output=fake_gpio)
        a.stop()

        assert ('gpio', 22, 0) in call_log, "SD pin never driven LOW"
        assert 'popen' in call_log, "aplay popen never called"
        low_idx = call_log.index(('gpio', 22, 0))
        popen_idx = call_log.index('popen')
        assert low_idx < popen_idx, \
            f"SD LOW at log[{low_idx}] but aplay started at log[{popen_idx}] — order wrong"

    def test_sd_pin_driven_high_after_warmup_write(self):
        """SD pin driven HIGH only after at least one warmup write has occurred."""
        write_count = [0]
        write_count_when_high = []

        local_proc = FakeProcess()
        original_side_effect = local_proc.stdin.write.side_effect

        def tracking_write(data):
            write_count[0] += 1
            if original_side_effect:
                original_side_effect(data)
        local_proc.stdin.write.side_effect = tracking_write

        def fake_gpio(pin, value):
            if value == 1:
                write_count_when_high.append(write_count[0])

        local_popen = MagicMock(side_effect=lambda *a, **kw: local_proc)
        a = SounddeviceAudio(sd_pin=22, _popen=local_popen, _gpio_output=fake_gpio)
        a.stop()

        assert len(write_count_when_high) >= 1, "SD pin never driven HIGH"
        assert write_count_when_high[0] >= 1, \
            "SD driven HIGH before any silence was written to aplay"

    def test_sd_pin_high_not_immediate(self):
        """SD pin driven HIGH only after a settle delay, not instantly after LOW."""
        timings = []
        start = time.monotonic()
        local_proc = FakeProcess()
        local_popen = MagicMock(side_effect=lambda *a, **kw: local_proc)
        ...
        def fake_gpio(pin, value):
            timings.append((value, time.monotonic() - start))

        a = SounddeviceAudio(sd_pin=22, _popen=local_popen, _gpio_output=fake_gpio)
        a.stop()

        high_times = [elapsed for val, elapsed in timings if val == 1]
        assert len(high_times) >= 1, "SD never driven HIGH"
        assert high_times[0] >= 0.05, \
            f"SD driven HIGH too quickly ({high_times[0]*1000:.0f} ms) — settle delay missing"

    def test_no_gpio_calls_when_sd_pin_not_set(self):
        """No GPIO calls at all when sd_pin is not provided."""
        gpio_calls = []

        def fake_gpio(pin, value):
            gpio_calls.append((pin, value))

        local_proc = FakeProcess()
        local_popen = MagicMock(side_effect=lambda *a, **kw: local_proc)
        a = SounddeviceAudio(_popen=local_popen, _gpio_output=fake_gpio)
        a.stop()

        assert gpio_calls == [], f"Unexpected GPIO calls when sd_pin=None: {gpio_calls}"
