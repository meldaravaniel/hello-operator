"""Tests for src/media_store.py — MediaStore local SQLite cache."""

import sqlite3

import pytest
from src.media_store import MediaStore, _KEY_PLAYLISTS, _KEY_ARTISTS, _KEY_GENRES
...
def playlists():
    return [
        MediaItem(media_key="/playlists/1", name="Chill Vibes", media_type="playlist"),
        MediaItem(media_key="/playlists/2", name="Rock Classics", media_type="playlist"),
    ]
...
def artists():
    return [
        MediaItem(media_key="/artists/10", name="The Beatles", media_type="artist"),
        MediaItem(media_key="/artists/11", name="Led Zeppelin", media_type="artist"),
    ]
...
def genres():
    return [
        MediaItem(media_key="/genre/jazz", name="Jazz", media_type="genre"),
        MediaItem(media_key="/genre/rock", name="Rock", media_type="genre"),
    ]
...
def make_store(db_path, media_client):
    return MediaStore(db_path=db_path, media_client=media_client)
...
class TestUpdateStrategy:
    ...
    def test_store_updates_on_successful_different_response(self, tmp_store_db, mock_plex, playlists):
        ...
        new_playlists = [MediaItem(media_key="/playlists/99", name="New List", media_type="playlist")]
...
class TestStaleData:
    ...
    def test_store_removes_item_on_playback_not_found(self, tmp_store_db, mock_plex, playlists):
        ...
        store.remove_item(item_to_remove.media_key)

        result = store.get_playlists()
        assert not any(p.media_key == item_to_remove.media_key for p in result)

    def test_store_updates_hasContent_after_removal(self, tmp_store_db):
        mock = MockMediaClient()
        item = MediaItem(media_key="/playlists/1", name="Only Playlist", media_type="playlist")
        mock.set_playlists([item])
        store = make_store(tmp_store_db, mock)
        store.get_playlists()
        assert store.playlists_has_content is True

        store.remove_item(item.media_key)
        assert store.playlists_has_content is False
...
class TestAlbumsPerArtist:
    ...
    def albums(self):
        return [
            MediaItem(media_key="/albums/1", name="Abbey Road", media_type="album"),
            MediaItem(media_key="/albums/2", name="Let It Be", media_type="album"),
        ]
    ...
    def test_store_updates_albums_on_successful_response(self, tmp_store_db, mock_plex, albums):
        ...
        new_albums = [MediaItem(media_key="/albums/99", name="New Album", media_type="album")]
...
class TestManualRefresh:
    ...
    def test_store_refresh_updates_on_success(self, tmp_store_db, mock_plex, playlists):
        ...
        new_playlists = [MediaItem(media_key="/playlists/99", name="New", media_type="playlist")]
        mock_plex.set_playlists(new_playlists)
        store.refresh()
...
def _write_raw(db_path: str, cache_key: str, data: str) -> None:
    """Directly write a raw data string into media_cache, bypassing MediaStore."""
    ...
    conn.execute(
        "INSERT INTO media_cache (cache_key, data, updated_at) VALUES (?, ?, '2024-01-01T00:00:00+00:00') "
...
class TestHasContentExistenceCheck:
    ...
    def test_playlists_has_content_true_when_nonempty(self, tmp_store_db):
        ...
        _write_raw(
            tmp_store_db,
            _KEY_PLAYLISTS,
            '[{"media_key": "1", "name": "A", "media_type": "playlist"}]',
        )
        assert store.playlists_has_content is True
    ...
    def test_artists_has_content_true_when_nonempty(self, tmp_store_db):
        ...
        _write_raw(
            tmp_store_db,
            _KEY_ARTISTS,
            '[{"media_key": "/artists/1", "name": "Beatles", "media_type": "artist"}]',
        )
    ...
    def test_genres_has_content_true_when_nonempty(self, tmp_store_db):
        ...
        _write_raw(
            tmp_store_db,
            _KEY_GENRES,
            '[{"media_key": "/genre/rock", "name": "Rock", "media_type": "genre"}]',
        )
